...
import { getAllHelpArticles, HelpArticleSummary } from '@/lib/sanity/queries'
...
export default async function HelpIndexPage({ searchParams }: { searchParams: { category?: string } }) {
  ...
  const shown = selected ? articles.filter(a => a.category === selected.id) : articles
  ...
  return (
    <div className="max-w-3xl px-6 py-16">
      ...
      {shown.length === 0 ? (
        <p className="text-sm text-ink-muted text-center py-12">No articles here yet.</p>
      ) : (
