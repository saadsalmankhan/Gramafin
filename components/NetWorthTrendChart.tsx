...
// Only the last point gets a permanent, filled dot (a small "you are here"
// marker) — every other point stays bare so the line itself carries the
// trend and the dot doesn't compete with it. Recharts' `dot` prop calls this
// per-point; returning an empty <g/> for everything but the last index is
// the standard way to make a per-point conditional dot.
function EndpointDot(props: { cx?: number; cy?: number; index?: number; lastIndex: number }) {
  const { cx, cy, index, lastIndex } = props
  if (index !== lastIndex || cx === undefined || cy === undefined) return <g />
  return (
    <circle cx={cx} cy={cy} r={4.5} fill={LINE_COLOR} stroke="white" strokeWidth={2} />
  )
}
